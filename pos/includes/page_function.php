...
// class pagination for prdid
class Paginationprdid{
	...
	function navPage($activePage, $amountPage){
		...
		if($activePage > 1){
			$prev = $activePage-1;
			$link_page .= "<span class=prevnext><a href=prdid.php?page=1><< First</a></span> 
		                    <span class=prevnext><a href=prdid.php?page=$prev>< Prev</a></span> ";
		}
		else{ 
			$link_page .= "<span class=disabled><< First</span> <span class=disabled>< Prev</span>";
		}
		...
		if ($i < 1)
			continue;
			$numeric .= "<a href=prdid.php?page=$i>$i</a>";
		}
		$numeric .= " <span class=current>$activePage</span> ";
	    ...
	    $numeric .= ($activePage+2<$amountPage ? " ... <a href=prdid.php?page=$amountPage>$amountPage</a> " : " ");
	    ...
	    if($activePage < $amountPage){
			$next = $activePage+1;
			$link_page .= " <span class=prevnext><a href=prdid.php?page=$next>Next ></a></span> 
		                     <span class=prevnext><a href=prdid.php?page=$amountPage>Last >></a></span> ";
		}
		else{
			$link_page .= "<span class=disabled>Next ></span><span class=disabled>Last >></span>";
		}
