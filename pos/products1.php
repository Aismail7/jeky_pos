	...
	if(isset($_GET['input']))
{
	...
	$input=$_GET['input'];
	echo $query=mysqli_query("select idprd,namaid,namasub from as_prdid where namaid LIKE '%input%'");
	...
	if($hasil>0){
		while ($data=mysqli_fetch_row($query)) {
			?>
			<a herf="javascript:autoInsert('<?=$data[0]?>','<?$data[1]?>''<?$data[2]?>';)">
				<?php
	   }		
	}
 }
	...
	elseif ($module == 'product' && $act == 'search')
	{
		...
		$queryprdid = "SELECT * FROM as_prdid WHERE status = 'Y' ORDER BY namaid ASC";
	...
	else 
	{
		...
		// showing up product data
		echo $queryProduct = "SELECT * FROM as_products ORDER BY productName ASC LIMIT $position,$limit";
		$sqlProduct = mysqli_query($connect, $queryProduct);
